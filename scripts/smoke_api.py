...
def _wait_for_job(client: TestClient, job_id: str, timeout_seconds: float = 60.0) -> dict:
    ...
    while time.time() < deadline:
        response = client.get(f"/v1/jobs/{job_id}", headers=_auth())
...
def main() -> int:
    ...
    path_resp = client.get(f"/v1/jobs/{job_id}/csv-path", headers=_auth())
    if path_resp.status_code != 200:
        print(f"[api-smoke] csv-path failed: {path_resp.status_code}")
        return 1

    dl_resp = client.get(f"/v1/jobs/{job_id}/download", headers=_auth())
